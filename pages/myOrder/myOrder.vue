<template>
	<view class="bg-grey my-order">
		<u-tabs-swiper ref="uTabs" :list="typeList" :is-scroll="true" active-color="#00BE87" inactive-color="#AAAAAA" :name="langType"
		...
		<swiper :current="swiperCurrent" @transition="transition" @animationfinish="animationfinish" class="swiper-container">
			<swiper-item class="swiper-item" v-for="(item, index) in typeList" :key="index">
				<order-show :type='item.type'></order-show>
			</swiper-item>
		</swiper>
	</view>
</template>
...
<script>
	...
	export default {
			...
			lang: {
				...
				handler: function(val) {
					uni.setNavigationBarTitle({
						title: this._i18n.messages[val].myOrder.pageName
					});
				}
			}
		},
		...
		methods: {
			queryTypeList() {
				const vm = this
				vm.$get(urls.queryOrderType, {}).then(res => {
					const urlType = getCurrentPages()[getCurrentPages().length - 1].options
					if (urlType.type) {
						const target = res.data.find(item => item.type === Number(urlType.type) && item.id === Number(urlType.id))
						vm.curType = res.data.indexOf(target)
